(function() {
    ...
    // Khởi tạo object theo dõi hiệu suất
    const adsPerformance = {};
    banners.forEach(banner => {
        adsPerformance[banner.id] = {
            impressions: 0,
            clicks: 0,
            name: banner.id
        };
    ...
    async function sendTelegramReport() {
        ...
        for (const id in adsPerformance) {
            const ad = adsPerformance[id];
        ...
        try {
            ...
            if (response.ok) {
                // Reset số liệu sau khi gửi báo cáo
                for (const id in adsPerformance) {
                    adsPerformance[id].impressions = 0;
                    adsPerformance[id].clicks = 0;
                }
            }
        } catch (error) {
    ...
    // Thiết lập timer gửi báo cáo
    setInterval(sendTelegramReport, REPORT_INTERVAL);
    ...
    if (validBanners.length > 0) {
        ...
        adsPerformance[banner.id].impressions++;
        ...
        bannerElement.innerHTML = `
                <a href="${banner.href}" 
                   ...
                   onclick="adsPerformance['${banner.id}'].clicks++">
                    <img src="${banner.src}" 
